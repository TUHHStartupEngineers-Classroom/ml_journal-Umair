################### Question 1: Leaderboard visualization ###################
...
############### Question 2: Tune a model with grid search ####################





deeplearning_h2o <- h2o.loadModel("04_Modeling/h20_models/challenge/GBM_4_AutoML_20210530_122753")
...
# Question 2: Visualize the trade of between the precision and the recall and the optimal threshold
...
stacked_ensemble_h2o <- h2o.loadModel("04_Modeling/h20_models/challenge/StackedEnsemble_AllModels_AutoML_20210530_122753")
deeplearning_h2o     <- h2o.loadModel("04_Modeling/h20_models/challenge/DeepLearning_grid__2_AutoML_20210530_122753_model_1")
glm_h2o              <- h2o.loadModel("04_Modeling/h20_models/challenge/GBM_grid__1_AutoML_20210531_150000_model_3")
...
path <- "04_Modeling/h20_models/challenge/StackedEnsemble_AllModels_AutoML_20210530_122753"
...
model_metrics_tbl <- fs::dir_info(path = "04_Modeling/h20_models/challenge/") %>%
  select(path) %>%
...
######################## Question 5: Precision vs Recall Plot ######################
...
ranked_predictions_tbl <- predictions_tbl %>%
  bind_cols(test_tbl) %>%
  select(predict:Yes, predict) %>%
  # Sorting from highest to lowest class probability
  arrange(desc(Yes))


ranked_predictions_tbl %>%
  mutate(ntile = ntile(Yes, n = 100)) %>%
  group_by(ntile) %>%
  summarise(
    cases = n(),
    responses = sum(predict == "Yes")
  ) %>%
  arrange(desc(ntile))
...
################ Question 8 Dashboard with cowplot ####################

############ combine all in one big function #################
